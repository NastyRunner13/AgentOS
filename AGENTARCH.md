...
`computer see` captures the primary monitor and the current master turn receives that image (downscaled, 0–1000 axes drawn on it, labeled **untrusted**). `computer click`/`type`/`keys`/`scroll` with `x`,`y` is the explicit pixels path: those numbers are **0–1000 on the attached image** (0,0 top-left, 1000,1000 bottom-right) — Gemini's native computer-use space. The kernel maps `screen_x = x/1000 * width` (and the same for y). Do not ask the model for image-pixel coordinates; mixing the two spaces is how taskbar clicks land on the status bar. After mapping, a point still outside the monitor is not **verified** and must not click. The next attached screenshot marks the last click with a red crosshair. `type`/`keys`/`scroll` with no `app` and no `x`,`y` are ring 1 focused-window calls: they require a prior pixels click (no screen-center fallback — the focused window may still be the editor) and `expect=` so the kernel can **verify**. Missing `expect` or no prior click does not type. `verified: false` without `stuck` is not **stuck**; master re-prompts and the model must continue or the turn ends. A failed `open` (after the ring-2 **card** and session grant when the app is not allowlisted) attaches a `see` screenshot; the operator does not halt. Automatic pixels fallback (vision ground when no `x`,`y` on an allowlisted app) still fires only when the a11y tree is empty. Full-desktop allowlist removal stays Phase 6.
...
**Transcript.** Each foreground turn prints: the user line (`❯` plus the text); a thinking spinner that collapses in place to `◆ thought <duration> <preview>` (truncated single-line model reasoning) when the first non-thought token or tool arrives; one row per tool (running is replaced by done: name, short args, **ring**, elapsed; `files` writes also show a short content preview; independent tools in one model step may run together and are matched by tool-call id); the assistant reply as rendered markdown, not raw markers; a blank line, then a footer rule with wall-clock time, tool count, and **card** count. The CLI subscribes to `tool.call`, `tool.result`, and `approval.resolved` as well as `agent.state` / `approval.request` / `error`. `/plan` shows the session's waiting plan, else the latest saved plan, else workspace `plan.md` when that file exists; it does not invent a plan. `/plan <id>` reprints a saved plan. `/plans` lists `data/plans/`.
...
| `computer` allowlisted app, or `see` / `snapshot` / `list_windows`, or `click`/`type`/`keys`/`scroll` with `x`,`y` and no app, or `type`/`keys`/`scroll` with no app (focused window / last click) | 1 | silent launch / screen read / coordinate act |
...
`config/kernel.yaml` `max_tool_steps` (16) caps billed tool rounds per chat turn. It is not a **loop** cap. `computer see` / `snapshot` / `list_windows` do not consume a billed step (a see→click→see chain still spends one step per click). Total model rounds are capped at `2 × max_tool_steps` so observe-only loops cannot run away. Browser snapshot text is clipped to `tool_result_max_chars` inside the **untrusted** wrap. This does not fix a model that stops after `verified: false`.

A model step may emit several tool calls. The kernel runs that batch in parallel when every call is **ring** 0–1, none is `skill` / `ask_user` / `browser` / `kb_consolidate` / a mutating `computer` action (`click`/`type`/`keys`/`scroll`/`open`/`close`/`focus`), and no two `files` calls share a path (read or write). Two `computer` observes do not share a group (they share the screenshot files). Ring ≥2 **cards**, GUI mutations, `skill` (still first), `ask_user`, `browser`, and `kb_consolidate` stay serial in that step. A second mutating `computer` action in the same step is skipped (`Screen state changed…`) as before. Tool results are appended in call order. `max_tool_steps` still counts the round, not each call.
...
`kernel.yaml`: `concurrent_slots`, `max_tool_steps` (chat billed-tool cap, 16; observe-only computer rounds exempt; hard round cap 2×), `clarify`, `tool_result_max_chars`, `skills_dir`.
